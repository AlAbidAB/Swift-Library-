...
//       }
    ...
    //            addShadowColorFromBackgroundImage()
                self.containerImageView.image = newValue
                 containerImageView = UIImageView(frame: UIScreen.main.bounds)
                containerImageView.contentMode =  UIView.ContentMode.scaleAspectFill
                self.containerView.insertSubview(containerImageView, at: 0)
               // self.containerView.sendSubviewToBack(containerView)
            }
        }
    ...
    //        addShadowColorFromBackgroundImage()
        ...
        private func addViewLayoutSubViews() {
            ...
            self.containerView.addSubview(self.containerImageView)
             self.containerView.sendSubviewToBack(self.containerImageView)
